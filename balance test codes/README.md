...
| :--- | :--- | :--- |
| **`balance_server.ino`** | Arduino/Firmware |Initial version for the balance test. |
| **`balance_server_extra_threshold.ino`** | Arduino/Firmware | **Final firmware version**.  This file should be uploaded to the microcontroller to handle sensor input, MQTT communication, and test timing|
...
2.  **Client Execution:** The **`balance_server_extra_threshold.ino`** firmware initiates the test sequence on the physical device.
